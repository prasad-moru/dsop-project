    ...
    <div class="cart-items-container">
      ...
      <div class="cart-items">
        <div class="cart-item" v-for="item in cartItems" :key="item.product.id">
          ...
          <div class="cart-column total">{{ getItemTotal(item) }}</div>
        </div>
      </div>
...
const removeFromCart = (item: CartItem) => {
  trackCustomAction('product_removed_from_cart', {
    ...
    cart_total_items: cartStore.count,
  })
...
const submitOrder = () => {
  ...
  fetch('/api/orders', {
    ...
    .catch((error: Error) => {
      ...
      trackError(error, { action: 'submit_order', customer_id: order.customerId })
      alert('Error occurred while submitting order')
    })
}
</script>
...
<style scoped>
/* (styles unchanged \u2014 keep existing styles from the original file) */
</style>
