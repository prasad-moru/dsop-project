...
func main() {
	...
	// Use Datadog middleware for tracing
	router.Use(gintrace.Middleware(getEnvVarWithDefault("DD_SERVICE", "makeline-service")))
...
// initDatadogTracer initializes the Datadog APM tracer
func initDatadogTracer() {
	ddAgentHost := getEnvVarWithDefault("DD_AGENT_HOST", "127.0.0.1")
	ddTraceAgentPort := getEnvVarWithDefault("DD_TRACE_AGENT_PORT", "8126")

	serviceName := getEnvVarWithDefault("DD_SERVICE", "makeline-service")
	env := getEnvVarWithDefault("DD_ENV", "production")
	version := getEnvVarWithDefault("DD_VERSION", os.Getenv("APP_VERSION"))

	tracer.Start(
		tracer.WithAgentAddr(ddAgentHost+":"+ddTraceAgentPort),
		tracer.WithService(serviceName),
		tracer.WithEnv(env),
		tracer.WithServiceVersion(version),
		tracer.WithRuntimeMetrics(),
		tracer.WithAnalytics(true),
		tracer.WithDebugMode(getEnvVarWithDefault("DD_TRACE_DEBUG", "false") == "true"),
	)

	log.Printf("Datadog tracer initialized: service=%s, env=%s, version=%s", serviceName, env, version)
}
...
// fetchOrders fetches orders from the order queue and stores them in database
...
// getOrder gets a single order from database by order ID
...
// getEnvVar gets an environment variable or exits if not set
...
// getEnvVarWithDefault gets an environment variable or returns a default value
...
// initDatabase initializes the database based on the API type
func initDatabase(apiType string) (*OrderService, error) {
	...
	case AZURE_COSMOS_DB_SQL_API:
		...
		cosmosRepo, err := NewCosmosDBOrderRepo(
			dbURI, dbName, containerName, dbPassword,
			PartitionKey{dbPartitionKey, dbPartitionValue},
		)
		if err != nil {
			return nil, err
	...
	default:
		...
		if useWorkloadIdentityAuth == "true" {
			...
			mongoRepo, err := NewMongoDBOrderRepoWithManagedIdentity(
				dbListConnStringsURL, dbName, collectionName,
			)
			if err != nil {
				return nil, err
			}
