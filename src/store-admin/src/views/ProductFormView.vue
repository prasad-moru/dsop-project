...
import { trackCustomAction, trackError } from '@/datadog-rum'
...
const generateDescription = (): void => {
  ...
  fetch(`${aiServiceUrl}/generate/description`, {
    ...
    .catch((error) => {
      console.log(error)
      trackError(error, { action: 'generate_description' })
...
const generateImage = (): void => {
  ...
  fetch(`${aiServiceUrl}/generate/image`, {
    ...
    .catch((error) => {
      console.log(error)
      trackError(error, { action: 'generate_image' })
...
const saveProduct = (): void => {
  ...
  fetch(`${productServiceUrl}`, {
    ...
    .then((data) => {
      ...
      trackCustomAction('product_updated', { product_id: data.id, product_name: data.name })
    } else {
      productStore.addProduct(data)
      trackCustomAction('product_added', { product_id: data.id, product_name: data.name })
    }
      router.push(`/product/${data.id}`)
    })
    .catch((error) => {
      trackError(error, { action: 'save_product' })
