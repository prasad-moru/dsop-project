...
def _handle_openai(user_prompt):
    ...
    api_key = os.environ.get("OPENAI_API_KEY", "").strip()   # add .strip()
    org_id = os.environ.get("OPENAI_ORG_ID", "").strip()     # add .strip()
    ...
    response = _create_completion(client, os.environ.get("OPENAI_CHAT_MODEL", "gpt-4o-mini"), user_prompt)
    return response.choices[0].message.content
